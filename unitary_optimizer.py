...
save_dir = 'C:/Users/yanzh/PycharmProjects/MIT_code/csv_files/pytorch_save'
save_csv = 'C:/Users/yanzh/PycharmProjects/MIT_code/csv_files/pytorch_save/csv_files'
...
class unitary_optimizer():
	...
	def format_time_tensor(self, time_tensor, grad_bool = True):
		...
		#times_tiled = times_tiled.to(device=device, dtype=dtype)

		# 单独设置 requires_grad 参数
		#if grad_bool:
		#	times_tiled.requires_grad_(True)

		#return times_tiled

	def construct_matrix(self, time_params, multiply_forward = True, forward_pass = True):
		if self.manual_grad_calc:
			self.matrix_exp, self.imaginary_matrices = batch_matrix_exp(self.control_matrices_real,
																		time_params,
																		return_imaginary_matrices = True)
		else:
	...
	def construct_dqas_matrix(self, num_layers=1, num_qubits=6, num_placeholders=10, learning_step=6, data_reuploading=True,
							  barrier=False, w_input=False, w_output=False, seed=1234):

		sphc_struc = []
		sphc_ranges = [[*range(num_qubits)] for _ in range(len(sphc_struc))]
		noisy = False
		# ops = {0: ("U3", [0]), 1: ("U3", [1]), 2: ("U3", [2]), 3: ("CU3-single", [0]), 4: ("CU3-single", [1]),
		#		   5: ("CU33", [0]), 6: ("CNOT", [0]), 7: ("CNOT", [1]), 8: ("CNOTT", [0]), 9: ("H", [2]), 10: ("E", [0, 1, 2])}
		ops = {0: ("RZ", [0])
			...
			, 6: ("HCNOT", [1]), 7: ("CNOTH", [1])
			, 8: ("E", [0, 1, 2])}
		cm = circuit_ud_matrix.Circuit_manager(sphc_struc=sphc_struc, sphc_ranges=sphc_ranges, num_qubits=num_qubits
											   , num_placeholders=num_placeholders, num_layers=num_layers, ops=ops,
											   noisy=noisy, learning_step=learning_step)

		# dqas = circuit_ud_matrix.circuit(cm=cm, data_reuploading=data_reuploading, barrier=barrier)

		qdqn_target = circuit_ud_matrix.dqas_new(cm=cm, w_input=w_input, w_output=w_output,
												 data_reuploading=data_reuploading, barrier=barrier, seed=seed)
		dqas_matrix = qdqn_target.forward(torch.tensor([0.5, -0.6, 0.8, 0.9]))
	...
	def optimize(self, n_times = 2, n_epochs = 2000, print_statistics = True, 
		...
		**kwargs):
		...
		if save_results:
			...
			timestamp = currentDT.strftime('%Y-%m-%d_%H-%M-%S-%f')
			save_csv = 'C:/Users/yanzh/PycharmProjects/pythonProject/asd/MIT_code/csv_files/'
			df.to_csv(save_csv + str(timestamp) + '.csv')
...
#			df.to_csv(save_csv + str(currentDT) + '.csv')
	...
	def ordered_matrix_multiply(self, matrices_in, multiply_forward=True):
		...
		device = config.device
		matrix_dqas_in = self.construct_dqas_matrix()
		if multiply_forward:
			for i in range(n_mats - 1):
				temp = torch.matmul(matrix_dqas_in, matrices[i])
				matrices[i + 1] = torch.matmul(matrices[i + 1], temp)
		# matrices[i+1] = torch.matmul(matrices[i+1], matrices[i])
			...
			shifted_matrices[-1, :, :] = torch.eye(matrices.size()[1], device=device, dtype=dtype)
			for i in range(n_mats - 1):
				temp = torch.matmul(matrix_dqas_in, matrices[-(i + 1)])
				shifted_matrices[-(i + 2)] = torch.matmul(shifted_matrices[-(i + 1)], temp)
		# shifted_matrices[-(i+2)] = torch.matmul(shifted_matrices[-(i+1)], matrices[-(i+1)])
...
def construct_dqas_matrix_batch(num_layers = 1, num_qubits = 6, num_placeholders = 10, learning_step = 5,data_reuploading = True,
								 barrier = False,w_input = False, w_output = False, seed = 1234):
	sphc_struc = []
	sphc_ranges = [[*range(num_qubits)] for _ in range(len(sphc_struc))]
	noisy = False
...
def batch_matrix_multiply(matrices):
	...
	device = config.device
	matrix_dqas_in = construct_dqas_matrix_batch()
	if n_mats == 1:
		return matrices

	for i in range(n_mats):
		matrices[i, :, :] = torch.matmul(matrices[i, :, :], matrix_dqas_in)

	# if odd number, multiply last matrices to get even number
	if n_mats % 2 == 1:
		#matrices[1,:,:] = torch.matmul(matrices[1,:,:], matrices[0,:,:])
		#matrices = matrices[1:, :, :]
		matrices[n_mats - 2, :, :] = torch.matmul(matrices[n_mats - 2, :, :], matrices[n_mats - 1, :, :])
		matrices = matrices[:n_mats - 1, :, :]
		n_mats = n_mats-1
