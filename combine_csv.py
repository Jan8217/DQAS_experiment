...
csv_list = glob.glob('C:/Users/yanzh/PycharmProjects/pythonProject/asd/MIT_code/csv_files/*.csv')
...
def format_pandas(df):
	...
	df_new.loc[:, 'end loss'] = df['loss'].iloc[-1]
	df_new.loc[:, 'total steps'] = df['gradient descent step'].iloc[-1]
	return df_new
