...
n_repeats = 1  # number of time to randomly repeat for given set of parameters
...
for i, ii in zip(dimensions, dimensions_raw):
	...
	for j in n_targets:
		for k in range(n_repeats):
			...
			a.optimize(n_epochs = 10, lr = lr/ii, weight_decay = 0.0, manual_grad_calc = True,
			...
			#a.greedy_optimize(max_n_times=100, epochs_per=1000, print_statistics=True,
			#			  manual_grad_calc=True, lr = lr/ii, init_type='zeros', optim_type='Adam', relative_stop_rate=1e-8,
			#			  absolute_stop_rate=1e-7, print_every=100, min_epochs=15, init_range=0.01,
			#			  normalize_learning_rate=1.0)
